...
% MIP settings for normality test (segmentation runs on MIP, not single plane).
% Defaults match Option 6 since the demo .nd2 is the same 5-FOV stack.
params.normality_use_mip         = true;
params.normality_series          = 1;      % FOV1 starts at Series 1
params.normality_series_end      = 15;     % FOV1 spans Series 1-15 (15 Z planes)
params.normality_nucleus_channel = 2;      % 405nm = plane 2 (channel order: 488,405,561,532)
...
params.gt_export_centroids_csv = true;  % toggle: export per-method segmented cell centroids CSV
params.gt_roi_enabled          = true;  % toggle: draw an interactive ROI before accuracy scoring
%   Option 6: ROI drawn on the 40X nucleus image before segmentation runs.
%   Option 7: ROI drawn on the NCC-refined 10X mosaic overlap after alignment.
%   In both cases only cells inside the ROI are counted in the final accuracy
%   metrics, eliminating false-misses from FOV edge mismatch.
%   Set to false to skip the interactive step and score the full region.
...
params.show_alignment_diagnostics = false;  % set true to pop up pre/post NCC + ROI overlap diagnostic plots
...
switch level_choice
    ...
    case 5
        % Rebuild img_data with MIP + correct DAPI channel so segmentation
        % runs on a focal-plane-independent projection instead of plane 1.
        fprintf('[INFO] Option 5 -- loading nucleus MIP for normality test...\n');
        fprintf('[INFO]   use_mip=%d  series=%d..%d  channel=%d\n', ...
                params.normality_use_mip, params.normality_series, ...
                params.normality_series_end, params.normality_nucleus_channel);
        norm_load_params                 = params;
        norm_load_params.use_mip         = params.normality_use_mip;
        norm_load_params.series          = params.normality_series;
        norm_load_params.series_end      = params.normality_series_end;
        norm_load_params.nucleus_channel = params.normality_nucleus_channel;
        norm_img_data = load_nd2(params.nd2_path, norm_load_params);
        fprintf('[INFO] img_data.z_loaded = %d  (0 = MIP, >0 = single plane)\n', ...
                norm_img_data.z_loaded);
        norm_results = run_nucleus_normality_test(norm_img_data, params);
    ...
    case 6
        ...
        fprintf('[INFO] gt_export_centroids_csv = %s\n', ...
                mat2str(params.gt_export_centroids_csv));
        if params.gt_export_centroids_csv
            fprintf('[INFO] Centroid CSV export + accuracy histogram ENABLED.\n');
        end
    ...
    case 7
        ...
        assignin('base', 'val_results', val_results);   % cache for Option 6 ROI reuse
        ...
        fprintf('[INFO] val_results cached -- run Option 6 next to reuse this ROI.\n');
        fprintf('[LOG]  Outputs saved to: %s\n', params.log_dir);
        diary off;
        return;
    otherwise
...
has_solidity = isfield(R.cells, 'solidity');
if ~has_solidity
    warning(['[SAVE] cells struct has no "solidity" field -- MATLAB may be ' ...
             'running a cached segment_nuclei.m. Run "clear functions" ' ...
             'and rerun Option 5 to pick up the disk version.']);
end
for k = 1:R.n_detected
    c = R.cells(k);
    if has_solidity
        solid_val = c.solidity;
    else
        solid_val = NaN;
    end
    fprintf(fid, '%d,%d,%s,%.4f,%.4f,%.4f,%.0f,"%s"\n', ...
            c.id, c.norm_class, c.norm_class_name, c.norm_confidence, ...
            c.circularity, solid_val, c.area, c.norm_reason);
end
fclose(fid);
