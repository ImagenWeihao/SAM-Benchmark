...
try
    ...
    px_10x = 0.65;
    warning('[10X]  Could not read pixel size, assuming %.4f µm/px', px_10x);
end
fprintf('[10X]  Pixel size: %.4f µm/px\n', px_10x);
...
fprintf('[10X]  Image size per FOV: %d x %d px  (%.1f x %.1f µm)\n', ...
...
try
    ...
    px_40x = 0.1625;
    warning('[40X]  Could not read pixel size, assuming %.4f µm/px', px_40x);
end
fprintf('[40X]  Pixel size: %.4f µm/px\n', px_40x);
...
fprintf('[40X]  MIP size: %d x %d px  (%.1f x %.1f µm)  across %d Z planes\n', ...
...
fprintf('[REG]  40X FOV physical size: %.1f x %.1f µm\n', W40*px_40x, H40*px_40x);
...
fprintf('[REG]  FOV centre    : (%.1f, %.1f) µm\n', cx_um,  cy_um);
...
fprintf('\n[DONE] Best match: 10X FOV %d  NCC=%.3f  centre=(%.1f, %.1f) µm\n\n', ...
